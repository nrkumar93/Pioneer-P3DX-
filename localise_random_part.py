...
def callback(sonar_data, pose_data):
	#print pose_data
	global bot_x_1_mat, bot_y_1_mat, bot_euler_1, x_1, y_1, alpha_1, world, count, wslow, wfast
	count+=1
	print count
	obj_x=[0.0]*8
	obj_y=[0.0]*8
	...
	for m in range(M):
		...
		for i in range(8):
			...
			if obstacle:
				...
				pz_r_hit=normalpdf(r_orig,zr,4)
				pz_theta_hit=normalpdf(theta_orig,ztheta,0.3)
	...
	rand_samp_num=int(max(0,1-wfast/wslow)*1000)
	print rand_samp_num
