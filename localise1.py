...
def callback(sonar_data, pose_data):
	#print pose_data
	global bot_x_1_mat, bot_y_1_mat, bot_euler_1, x_1, y_1, alpha_1, world, count
	count+=1
	obj_x=[0.0]*8
	obj_y=[0.0]*8
	r=[0.0]*8
	...
	for m in range(M):
		...
		x[m]=int(x[m]+gauss(0,2))
		y[m]=int(y[m]+gauss(0,2))
		...
		for i in range(8):
			...
			if obstacle:
				...
				pz_r_hit=normalpdf(r_orig,zr,6)
				pz_theta_hit=normalpdf(theta_orig,ztheta,0.5)
