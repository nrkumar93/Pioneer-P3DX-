...
it assumes that the odometry is perfect
...
def rotate(rot,bot_euler): 
	...
	twist=Twist()
	print abs(bot_euler)-pi
	print bot_euler
	if rot=="north":
		twist.angular.z=abs((bot_euler)-pi/2)
	elif rot=="south":
		twist.angular.z=(abs((bot_euler)+(pi/2)))
	elif rot=="east":
		twist.angular.z=(abs(bot_euler))
	elif rot=="west":
		twist.angular.z=(abs(abs(bot_euler)-pi))
	pub.publish(twist)
	rospy.sleep(0.2)
	twist.angular.z=0.0
	pub.publish(twist)
...
def callback(pose_data):
	...
	if bot_x_mat==next_pos_x and bot_y_mat==next_pos_y:
		...
		print rot, bot_euler
		if rot=="north":
			if not (bot_euler<(pi/2+0.02) and bot_euler>(pi/2-0.02)):
				rotate(rot,bot_euler)
			else:
				move()
		if rot=="south":
			if not ( bot_euler>(-1*pi/2-0.02) and bot_euler<(-1*pi/2+0.02)):
				rotate(rot,bot_euler)
			else:
				move()
		if rot=="east":
			if not ( bot_euler<0.02 and bot_euler>-0.02):
				rotate(rot,bot_euler)
			else:
				move()
		if rot=="west":
			if not ( bot_euler>(pi-0.02) or bot_euler<(-1*pi+0.02)):
				rotate(rot,bot_euler)
			else:
				move()
